    ...
    <AspectRatio ratio={3424 / 1286}>
      <div>
        <Box.FullAbs as={Media} at="mobile">
          <BgImage
            image={mountBottomMdImage}
            style={{ width: "100%", height: "100%" }}
          />
        </Box.FullAbs>
        <Box.FullAbs as={Media} greaterThan="mobile">
          <BgImage
            image={mountBottomLgImage}
            style={{ width: "100%", height: "100%" }}
          />
        </Box.FullAbs>
      </div>
...
const HomePage = ({
  ...
  data: { mountTopMd, mountTopLg, mountBottomMd, mountBottomLg },
  gatsbyImages,
}) => {
  useShowHeader();
  const trashes = useMemo(
    () =>
      JSON.parse(trasheSrc).map((t) => ({
        ...t,
  ...
  const [trashMx, trashMt, trashWidth] = useMemo(() => {
    const scaleRatio = Math.min(
      ...
      3000 / windowSize.width
    );
    const titleClearance =
      (containerWidth / titleRatio) * (isMobile ? 1.7 : 1) + 60;
    const mountTopHeight =
      (containerWidth / mountRatio) * scaleRatio * (220 / 1843);
    return [
              ...
              {trash && (
                ...
                >
                  <Box.Relative transform="rotate(7deg)">
                    <GatsbyImage
                      image={trash.gatsbyImg.regular}
                      alt={trash.name}
                    />
                    <Box.FullAbs>
                      <Face
                        id={trash.transform.faceNo}
                        transform={trash.transform.face}
                      />
                    </Box.FullAbs>
                  </Box.Relative>
                </Box.Absolute>
  ...
  const init = () => {
    ...
    timeline.to(
      heroTrashRef.current.querySelector(".trash"),
      {
    ...
    timeline.to(
      trashMountRef.current.querySelector(".margin-adj"),
      {
        marginTop: 0,
      },
      0
    ...
    timeline.to(
      trashMountRef.current,
      {
        y: `-=${trashWidth * (isMobile ? (isIos ? 0.27 : 0.3) : 0.22)}`,
      },
      scrollingDuration
    );
    timeline.to(
      ...
      {
        opacity: 0,
        top: `-=${trashWidth * (isMobile ? (isIos ? 0.27 : 0.3) : 0.22)}`,
      },
      scrollingDuration
    );
    ...
    // }, scrollingDuration)
    timeline.to(
      trashMountRef.current.querySelector(".trash-mount"),
      {
        opacity: 0.3,
      },
      scrollingDuration
    ...
    timeline2.to(
      trashMountRef.current.querySelectorAll(".trash-bubble"),
      {
        stagger: 0.25,
        scale: 1,
        ease: "back.out(1.7)",
    ...
    pages.forEach((p, i) => {
      if (i && pageRefs[i - 1].current) {
        timeline.to(
          pageRefs[i - 1].current.querySelector(".scroll-btn"),
          {
            opacity: 0,
  ...
  return (
    <Wrapper className="home-bg" bg="colors.yellow" height="100%">
          ...
          <ReactFullpage
            ...
            onLeave={(origin, destination) => {
              ...
              if (destination.isLast) {
                setTimeout(
                  () => navigate(`${isEn ? "/en" : ""}/catalogue/`),
                  scrollingDuration * 500
                );
              }
              if (timeline) {
                timeline.tweenTo(destination.index * scrollingDuration, {
                  duration:
                    (destination.index === 3 ? 2 : 1) * scrollingDuration,
                });
              }
              if (timeline2) {
      ...
      >
        <Box
          ml={`${trashMx - windowSize.width * 0.04}px`}
          mr={`${trashMx + windowSize.width * 0.04}px`}
          mt={`${trashMt}px`}
          className="margin-adj"
        >
      ...
      >
        ...
        >
          <Box.Relative transform="rotate(7deg)" className="trash">
            <GatsbyImage
              image={trashes[0].gatsbyImg.regular}
              alt={trashes[0].name}
            />
            <Box.FullAbs>
              <Face
                className="face"
                id={trashes[0].transform.faceNo}
                transform={trashes[0].transform.face}
              />
            </Box.FullAbs>
          </Box.Relative>
