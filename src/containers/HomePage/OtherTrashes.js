...
const OtherTrashes = ({ isEn, isMobile, trashes, onLoad = () => {} }) => {
  useEffect(() => {
    onLoad()
  }, [])
  return (
    <>
      ...
      <Box.Absolute width="9%" left={responsive('40%', '36%', '40%')} top="40%">
        <Box.Relative transform={responsive('rotate(-40deg)', 'rotate(0deg)')}>
          <Trash data={trashes[2]} noFace={isMobile} />
        </Box.Relative>
        <Box.Absolute
          top={isEn ? '-42.5%' : "-28%"}
          left={isEn ? '27.5%' : "48%"}
