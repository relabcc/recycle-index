...
const TrashNote = ({ children, ...props }) => {
  ...
  return children && (
    <Box.Absolute top={responsive('4em', 'auto')} bottom={responsive('auto', '1.25em')} right={responsive('6%', '4.75em')} width={responsive('50%', '33%')}>
...
const getPoses = (data, yStart, explosionGap) => {
  const posByPartName = {}
  return [
    data.imgs.map((cfg) => {
      // calc parts y position
      const yPos = `${(yStart - cfg.y - data.positions[cfg.order] - imgSize[1] * explosionGap / 100 * cfg.order) / imgSize[1] * 100}%`
      if (cfg.partName) {
...
const TrashPage = ({ data, allData }) => {
  ...
  const pages = [
    ...
    <>
          ...
          <Flex pt="1em" flexDirection={responsive('column', 'row')}>
            ...
            {data.alternative && (
              ...
              >
                ...
                <Flex width={responsive('60em', '22em')} mr={responsive('2em', '1em')} alignItems="flex-end" my={responsive('2em', '0.5em')}>
                  <Box width="22%" pb="5%">
                    <Image src={planb} />
                  </Box>
                  <Box flex="1" pl="2%">
      ...
      {scrollProgress >= 1 && (
        <Box.Absolute left="0" right="0" top="0" height={`${windowSize.height - theme.headerHeight}px)`} pointerEvents="none">
          <Container height="100%">
  ...
  useEffect(() => {
    ...
    const [poses, posByPartName] = getPoses(data, yStart, explosionGap)
    const combineParts = {}
    ...
    data.imgs.forEach((cfg, i) => {
      ...
      if (cfg.partName) {
        ...
        if (/^@/.test(data.belongsTo[cfg.partName])) {
          combineParts[cfg.order] = data.belongsTo[cfg.partName].substring(1)
        }
      }
      theTimeline.to(layerRefs[cfg.index].current, {
        y: 0,
    ...
    data.imgs.forEach((cfg) => {
      if (combineParts[cfg.order]) {
        theTimeline.to(layerRefs[cfg.index].current, {
          y: posByPartName[combineParts[cfg.order]],
          duration: scrollingDuration,
        }, 2 * scrollingDuration)
        if (cfg.partName) {
          theTimeline.to(partsRefs[cfg.index].current.querySelector('.circle-container'), {
          ...
          theTimeline.to(partsRefs[cfg.index].current.querySelector('.line'), {
            opacity: 0,
            css: cfgPoses[cfg.partName].linePos,
            duration: scrollingDuration,
          }, scrollingDuration * 2)
          theTimeline.to(partsRefs[cfg.index].current.querySelector('.circle-1'), {
            opacity: 0,
