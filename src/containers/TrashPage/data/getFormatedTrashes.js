...
const lineParser = (d) => {
  const lines = d
    ...
    .split("\n");
  return fromPairs(
    lines.map((l) => l.split("-").map((w) => w.trim().replace(/\|/g, "\n")))
  );
};
...
const rateParser = (d) => {
  ...
  return fromPairs(
    lines.reduce((all, l) => {
      ...
      const name = splitted[0]?.trim().replace(/\|/g, "\n");
      const cleanName = splitted[0]?.trim().replace(/\|/g, "");
      const rate = splitted[1]?.trim();
      return [
        ...all,
        [name, Math.round(parseFloat(rate))],
        [cleanName, Math.round(parseFloat(rate))],
        [`${name}*`, Math.round(parseFloat(rate))],
      ];
    }, [])
  );
...
const remapKeys = (data, keyMap) =>
  data.map((dd) => {
    const mapped = mapValues(
      ...
      (v, k) => (transformer[k] ? transformer[k](v) : v)
    );
    return mapped;
  });
...
module.exports = (data, scale = [], cfg = []) => {
  ...
  const transformed = remapKeys(data, dataKeys)
    .filter((d) => d.id)
    .map((d) => ({
      ...d,
