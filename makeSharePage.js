...
const images = require('./src/containers/TrashPage/data/images/index.png')
const writeFile = util.promisify(fs.writeFile)

const family = 'Noto Sans CJK TC';
registerFont(path.resolve(__dirname, 'fonts/NotoSansCJKtc-Black.otf'), { family });
const imgSize = [850, 624]

const WIDTH = 1200;
const HEIGHT = 630;
const noto = 'Noto Sans CJK TC';
...
const remapKeys = (data, keyMap) => data.map((dd, id) => ({
  id,
...
const createOg = async (data) => {
  const shape = await loadImage(path.resolve(__dirname, 'src/containers/TrashPage/share-bg.svg'))
  const logo = await loadImage(path.resolve(__dirname, 'src/containers/logo.svg'))
  const trash = await loadImage(data.img)
  ...
  const s = data.transform.shareScale / 100 * 1.1
...
Promise.all(['scale', 'data']
  ...
  const transformed = remapKeys(data, dataKeys).map(d => ({
    ...d,
    img: get(images, [d.name, d.name]),
    transform: mapValues(scales[d.name], d => isNaN(d) ? d : d * 1),
  }))

  return Promise.all(transformed.map(createOg)).then(process.exit)
})
